...
3D 뷰 조작은 실행 중인 AM 내부에서만 가능합니다(웹/standalone 불가). 두 가지 방법:

- 웹 ③의 **[🖼️ 선택 요소를 3D 뷰에 ADD (명령 만들기)]** → `ADD CE` 또는 `ADD <ref>` 명령을 만들어 줌 → AM 명령창에 붙여넣기
- AM 에서 요소 선택 후 `am-add-ce.pmlmac` 실행 (한 줄 `ADD CE`). 자주 쓰면 AM 툴바/Alias 에 등록해 버튼처럼 사용.
